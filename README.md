...
AIへの指示は **`plugin/` という1つのプラグイン**にまとめてあり、3つの skill
（`refine-pbi` / `implement-step` / `verify-e2e`）を `plugin/skills/<name>/SKILL.md`
として同梱しています。各ランタイムにインストールし、**名前で起動**します。

| ランタイム | インストール先 | 起動構文 | 備考 |
| ---------- | -------------- | -------- | ---- |
| Claude Code | `~/.claude/plugins/schritt` | `/schritt:refine-pbi <dir>` | プラグイン（skill 名は名前空間付き） |
...
```sh
# Claude Code 用: plugin/ を ~/.claude/plugins/schritt にシンボリックリンク
make install-plugin
...
plugin/skills/<name>/     refine-pbi / implement-step / verify-e2e の各 SKILL.md（プラグイン同梱）。
