...
> `scripts/install-codex.sh`（codex）でインストールされます（両 skill をまとめて設置）。
...
`refine.Refiner` / `implement.Implementer` を差し替え口にしているため、AI 呼び出し方法を
変えても上位層は影響を受けません。テストや `--demo` は Demo 実装を使います。エンジン固有の
起動引数（サンドボックス・ディレクトリ付与・`/name` vs `$name`）は `internal/agent` に集約
してあり、refine と implement の両段階で共有します。

## 今後のステップ（予定）

実装に続けて、`pre-exec review`（実装計画レビュー）・検証・PR作成のサブコマンドを、
同じ `cmd` / `internal/agent` / `internal/store` の構造に沿って追加していく想定です。
実装レポートを TUI で閲覧する機能も追加予定です。
